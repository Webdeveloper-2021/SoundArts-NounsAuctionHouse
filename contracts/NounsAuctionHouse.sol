...
constract NounsAuctionHouse is INounsAuctionHouse, Pausable, ReentrancyGuard, Ownable {
    ...
    function createAuction(uint256 id) external onlyOwner {
        _unpause();
     ...
     */
    function _createAuction(uint256 nounId) internal {
        try nouns.ownerOf(nounId) returns (address ownerAddress) {

            require(ownerAddress == address(this), "_createAuction: Token not in auction contract");
